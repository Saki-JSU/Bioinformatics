...
## read tpm data
tpm.rsem <- read.table("../data/tpm_original.txt", sep = "\t")
## read results in the last step
load("sceset_ct.RData")
mat_ct <- sceset_ct@assays@data$exprs
fd_ct <- rowData(sceset_ct)
pd_ct <- colData(sceset_ct)
## read data of normal single breast cells
nick_normalize <- read.table("../data/RSEM_TPM_240_NormalCells.txt", sep = "\t", header = TRUE)
...
ht_cnas_chroms_high <- ha_rows_chroms + 
  Heatmap(mat_normepith_cna, 
          ...
          split = splits_chroms, 
          column_title = "Norm", column_title_gp = gpar(fontsize = 10),
          # top_annotation = ha_cnas_normepith,
          heatmap_legend_param = list(title = paste("Expression \n Score"), title_position = "topcenter", color_bar = "continuous",
  ...
  Heatmap(mats_epith_cna[[1]],
          ...
          # top_annotation = ha_cnas_epith_sep[[1]],
  ...
  Heatmap(mats_epith_cna[[2]],
          ...
          # top_annotation = ha_cnas_epith_sep[[2]],
...
draw(ht_cnas_chroms_high, gap = unit(0.2, "cm"), heatmap_legend_side = "right")
#dev.off()
